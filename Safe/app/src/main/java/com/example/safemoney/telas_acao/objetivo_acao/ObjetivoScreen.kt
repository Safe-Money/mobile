            ...
            ButtonsRow(
                ...
                onAddButtonClick = {
                    val objetivo = Objetivos(
                        nome = apelido.value,
                        urlImagem = url.value,
                        dataInicio = dataInicio.value.toBrazilianDateFormat17(),
                        ultimoDeposito = dataInicio.value.toBrazilianDateFormat17(),
                        dataTermino = dataFinal.value.toBrazilianDateFormat17(),
                        concluida = 0,
                        valorInvestido = 0.0,
                        valorFinal = valor.value.toDoubleOrNull() ?: 0.0,
                        fkUsuario = FkUsuario12(id = userId)
                    )
                    objetivoViewModel.cadastrarObjetivo(objetivo)
